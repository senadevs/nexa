...
- Loader (`site-transitions.ts`): solo el logo sobre ink, una vez por sesión, se puede saltar con clic o tecla y no aparece con movimiento reducido. Salida en un solo movimiento: el logo sale subiendo (0,5 s), a los 0,28 s el panel sube con el borde inferior curvado (1 s, `power4.inOut`) y a los 0,55 s arranca la entrada del hero, que estaba oculto desde el primer pintado (`html.intro-pending`) para que nunca se vea y se vuelva a esconder. `scrollbar-gutter: stable` evita el salto lateral al desbloquear el scroll. La red de seguridad inline (8 s) solo actúa si el script no llega a ejecutarse.
